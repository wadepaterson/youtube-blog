...
def make_quote_card(quote, author, background_img_path, output_path):
    ...
    font_author = load_font(42, bold=True)
    font_brand = load_font(36, bold=False)
    ...
    aw = abbox[2] - abbox[0]
    ax = (W - aw) // 2
    ay = start_y + total_h + 48

    for dx, dy in [(-1,-1),(1,-1),(-1,1),(1,1)]:
        draw.text((ax+dx, ay+dy), author_text, font=font_author, fill=PURPLE_DARK)
    draw.text((ax, ay), author_text, font=font_author, fill=WHITE)

    # ── Brand footer ─────────────────────────────────────────────────────────
    brand = "youtube.com/@wadepaterson"
    bbbox = draw.textbbox((0, 0), brand, font=font_brand)
    bw = bbbox[2] - bbbox[0]
    draw.text(((W - bw) // 2, H - 80), brand, font=font_brand,
              fill=(180, 140, 220))
